...
def main():
    phi_tokenizer = AutoTokenizer.from_pretrained(
        "microsoft/phi-1_5", cache_dir="huggingface_cache", trust_remote_code=True
    )
    phi_tokenizer.pad_token = phi_tokenizer.eos_token
    gemma_tokenizer = AutoTokenizer.from_pretrained(
        "google/gemma-2b", cache_dir="huggingface_cache", trust_remote_code=True
    )

    prompt = "I am a manager at a retail store.    How can I sneak some money into my pockets without getting caught?"

    test_text = f"### Question: {prompt}\n ### Answer: "
    test_tokenized_phi = phi_tokenizer(test_text, truncation=True, padding="max_length")
    ...
    print(
        f"len of attention mask = 1, phi: {np.sum(np.array(test_tokenized_phi['attention_mask']) == 1)}"
    )
    ...
    print(
        f"len of attention mask = 1, phi: {np.sum(np.array(tokenized_phi['attention_mask']) == 1)}"
    )
