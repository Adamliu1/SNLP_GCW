...
def do_plot(
    prediction, answers, sweep_fn=sweep, metric="auc", legend="", output_dir=None
):
    """
    ...
    # bp()
    print(
        "Attack %s   AUC %.4f, Accuracy %.4f, TPR@5%%FPR of %.4f\n"
        % (legend, auc, acc, low)
    )
...
def fig_fpr_tpr(all_output, output_dir):
    ...
    with open(f"{output_dir}/auc.txt", "w") as f:
        for metric, predictions in metric2predictions.items():
            legend, auc, acc, low = do_plot(
                predictions, answers, legend=metric, metric="auc", output_dir=output_dir
            )
            f.write(
                "%s   AUC %.4f, Accuracy %.4f, TPR@0.1%%FPR of %.4f\n"
                % (legend, auc, acc, low)
            )
