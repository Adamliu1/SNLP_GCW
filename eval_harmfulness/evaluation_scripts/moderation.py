...
class Moderation(nn.Module):
    ...
    def predict(
        ...
        text = [
            (
                t + self.tokenizer.eos_token
                if not t.endswith(self.tokenizer.eos_token)
                else t
            )
            for t in text
        ]
...
class QAModeration(Moderation):
    ...
    def predict(  # pylint: disable=arguments-differ,arguments-renamed
        ...
        if isinstance(question, str) != isinstance(answer, str):
            raise ValueError(
                "`question` and `answer` must be both str or be both list of str"
            )
        ...
        if batched_input:
            if len(question) != len(answer):
                raise ValueError(
                    "The `question` and `answer` lists must have the same length."
                )
