...
def apply_majority_voting_and_calculate_ratios(model_data):
    ...
    for model, prompts in model_data.items():
        ...
        for prompt, flags in prompts.items():
            ...
            results.append({'model_name': model, 'prompt': prompt, 'flagged': final_flagged})
...
def save_results_to_csv(results, model_ratios, output_dir):
    ...
    df_results.to_csv(os.path.join(output_dir, 'detailed_flagged_prompts.csv'), index=False)
    
    # Save model ratios
    df_ratios = pd.DataFrame(list(model_ratios.items()), columns=['model_name', 'flagged/all'])
